...
public class AlgoritmoResolucion {
    ...
    /** 
     * @param cB
     * @param k.getCasillaKakuro(padreVerticalCoord.getFirst()
     * @param k.getCasillaKakuro(padreHorizontalCoord.getFirst()
     * @param calcularCandidatos(padreVertical
     * @param padreHorizontal
     * @param posRelativaV
     * @param 0
     * @return int
     */
    ...
    /** 
     * @param numCell
     * @param resultCandidatos
     * @param partialCandidatos
     */
    ...
    private static boolean backtrackingFiltroSecuencias(Casilla c, Kakuro k, boolean vertical) {
        ...
        /* El filtro polinomico no ha funcionado :(
        if (vertical) {
            if (c.getSecuenciasCandidatasVertical().size() >= 3) {
                if ( polinomicFiltering(c, k, vertical) ) changes = true;
            }
        } else {
            if (c.getSecuenciasCandidatasHorizontal().size() >= 3) {
                if ( polinomicFiltering(c, k, vertical) ) changes = true;
            }
        }
        */
     ...
     */
    /*
    private static boolean polinomicFiltering(Casilla c, Kakuro k, Boolean vertical) {
        ...
        while (it.hasNext()) {
            ArrayList<Integer> secuencia = it.next(); //por cada secuencia que era candidata
            EnumSet<Candidatos> secuenciaSet = EnumSet.noneOf(Candidatos.class);
            for (int x : secuencia) {
                secuenciaSet.add(Candidatos.intTOenum(x));
            }
            int posV = c.getPosFirst();
            int posH = c.getPosSecond();
            EnumSet<Candidatos> noAparecen = secuenciaSet.clone(); //caso base, ningun numero de la sequencia
            Boolean earlyExit = false;
            int i = 1;
            if (vertical) {
                while ( i <= numCell && !earlyExit ) {
                    Casilla cB = k.getCasillaKakuro(posV+i, posH);
                    noAparecen.removeAll(cB.getCandidatosSet()); //noAparecen mira si hay elementos (de la secuencia) que nunca aparecen en las cB
                    EnumSet<Candidatos> interseccion = cB.getCandidatosSet();
                    interseccion.retainAll(secuenciaSet); //interseccion mira que todas las cB tengan elementos de la secuencia
            ...
            } else { //solo cambia que la i itera sobre posH como de costumbre
                while ( i <= numCell && !earlyExit ) {
                    Casilla cB = k.getCasillaKakuro(posV, posH+i);
                    noAparecen.removeAll(cB.getCandidatosSet()); //noAparecen mira si hay elementos (de la secuencia) que nunca aparecen en las cB
                    EnumSet<Candidatos> interseccion = cB.getCandidatosSet();
            ...
            if (!noAparecen.isEmpty() || earlyExit) {
                changes = true;
                it.remove();
            }
        }
        if (vertical) {
            c.setSecuenciasCandidatasVertical(secuencias_por_revisar);
        } else {
            c.setSecuenciasCandidatasHorizontal(secuencias_por_revisar);
        }
        return changes;
    }
    */
